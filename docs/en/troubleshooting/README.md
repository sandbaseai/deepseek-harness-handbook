...
| a custom OpenAI-compatible provider sends `/messages` after reusing an installed catalog id | provider-id collision / omitted protocol / delegated catalog stream | provider dict key, explicit-or-omitted `api`, sanitized baseURL, final request path, and fresh-Session A/B |
