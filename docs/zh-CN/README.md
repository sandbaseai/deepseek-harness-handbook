...
目前包含 **173 篇英文 canonical 指南、202 份多语言文档和 52 个精选 Awesome 资源**，所有版本敏感结论都标注验证日期和上游源码；简体中文提供核心内容与任务导航，下面五个最新专题摘要已逐行复核。英文原文始终是事实来源，中文入口不会假装拥有完整翻译覆盖率。

如果希望用中文交流使用技巧、插件开发和版本动态，可以查看上游公布的[飞书交流群讨论](https://github.com/deepseek-ai/deepseek-harness/discussions/4783)，以获取当前邀请链接和上下文。该群由社区自发组织，不代表 DeepSeek 官方支持；请勿分享凭据、私有 Session 日志或敏感工作区数据。
...
- [自定义 Provider 的多协议模型不可见](../en/getting-started/model-providers.md#a-custom-provider-catalog-is-not-automatically-multi-protocol)：区分实时模型目录与 Provider 默认协议筛选，为每个模型声明协议并验证真实请求端点。
